...
#%%
def get_test_data(site, PDP_days, start_search, end_search, fraction_test=0.5):
    means=[]
    maxes = []
    for day in PDP_days:
        start, end = get_window_of_day(day)
        weather_mean =  get_weather(site, start, end, agg='MEAN', window='24h')
        means.append(weather_mean)
        weather_max =  get_weather(site, start, end, agg='MAX', window='24h')
    ...
    # print(start_search,end_search)
    # start_search = pytz.timezone("US/Pacific").localize(datetime.datetime(year=start_search.year, month=start_search.month, day=start_search.day, hour=start_search.hour, minute=0))
    # end_search = pytz.timezone("US/Pacific").localize(datetime.datetime(year=end_search.year, month=end_search.month, day=end_search.day, hour=end_search.hour, minute=0))
    # print(start_search,end_search)
    weather_mean_all =  get_weather(site, start_search, end_search, agg='MEAN', window='24h')
    weather_mean = weather_mean_all.mean(axis=1)
    weather_max_all =  get_weather(site, start_search, end_search, agg='MAX', window='24h')
